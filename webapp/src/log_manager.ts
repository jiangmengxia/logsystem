...
class LogManager {
    ...
    async init(DBReady: () => void) {
        this.logDB = new LogDB(log_configer.get('DBName'), async () => {
            //每次登陆后先进行清理
            DBReady && DBReady()
            await this.reportImmediateLogsUnReported()
    ...
    async reportDaily_() {
        ...
        const logs: Array<DBLogItem> = await this.logDB.getLogsByReportName(days.map(day => { return day.logDay })) || []
        return this.reportLogs(logs, async (res) => {
            console.log('reportDaily_', res)
            let removedLogs = []
            for (let i = 0; i < res.length; i++) {
                if (res[i].ret === RET.SUCCESS) {
                    removedLogs.push(logs[i])
            ...
            await this.logDB.incrementalDeleteLogs(removedLogs, days) //删除当日清除的日志信息和日志日期信息
    ...
    /**
     * @description 日志上报请求
     * @param logs 待上传的日志   根据isOneByOne进行单条上传还是一次性上传
     * @param callback 
     */
    ...
    async reportLog(log: DBLogItem, callback: Function) {
        return await this.logReporter.report({ data: log })
    }
    ...
    async reportImmediately(logItems: Array<LogItem>) {
        console.log('reportImmediately', logItems)
